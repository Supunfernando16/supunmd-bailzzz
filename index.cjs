...
/**
 * supunmd-bail — CJS wrapper
 *
 * Destructure dan langsung pakai tanpa await ready:
 *
 *   const { makeWASocket, useMultiFileAuthState, Browsers } = require('supunmd-bail');
 *
...
function _wrapFn(name) {
  return async function(...args) {
    if (_loadError) throw new Error('[SUPUNMD-BAIL] Load failed: ' + _loadError.message);
    if (!_mod) await _load();
    const fn = name === 'makeWASocket' ? _mod.default : _mod[name];
    if (typeof fn !== 'function') throw new Error('[SUPUNMD-BAIL] "' + name + '" bukan fungsi');
    return fn(...args);
  };
}
...
function _wrapMakeWASocket() {
  return function makeWASocket(...args) {
    ...
    if (_loadError) throw new Error('[SUPUNMD-BAIL] Load failed: ' + _loadError.message);
    throw new Error(
      '[SUPUNMD-BAIL] makeWASocket dipanggil sebelum Baileys selesai load.\n' +
